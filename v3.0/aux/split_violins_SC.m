function split_violins_SC
% violin_single_cell, split for two samples
...
%     figwidth=(1)*scnsize(4);
    hfig{n}.Position=[0 0 scnsize(4)*0.9*(8.5/11) scnsize(4)*0.9];
    hfig{n}.Units='inches';
...
for j=1:num_signals;
    fignchooser=ceil(j/8);
    figure(hfig{fignchooser})
    k=j-(8*(fignchooser-1));
    haxis.(signal_names{j})=subplot(4,2,k,'Parent',hfig{fignchooser});
    for i=1:num_groups
        %Set cofactor for normalization based on threshold
%         thresh=Thresholds.(true_groups{i}){1,j};
%         cofactor=thresh*0.8;
        %Data is already transformed!
%         signal_data{i}=asinh(Data.(true_groups{i})(:,j)/cofactor);
        signal_data{i}=Data.(true_groups{i})(:,j).(signal_names{j});
        yline(:,i)=thresh;
        oncount(i)=(mean(signal_data{i}>thresh))*100;
        ontext{i}=sprintf('%.2f%%',oncount(i));
%         ontext{i}=sprintf('%.2f%%',OnCounts.(true_groups{i}).(signal_names{j})(2));
        if i==1;
            orien='left';
            division=[2 1];
        elseif i==2
        ...
        distributionPlot(signal_data{i},'globalNorm',0,'histOpt',1,'divFactor',2,...
        'xNames',[],'showMM',0,...
        'color',graph_colors(i),'widthDiv',division,'histOri',orien);
    pcntages(i)=sum(signal_data{i}>yline(1,i))/length(signal_data{i});
    end
%     fprintf('%s %.2f and %.2f\n',signal_names{j},pcntages(1)*100, pcntages(2)*100);
...
%         'color',graph_colors(1:num_groups));
    ...
    %     ytext(:)=max(cellfun(@max,signal_data))*0.9;
    currax=gca;
    ytext(:)=currax.YLim(1)*.9;
    text(xtext,ytext,ontext,'horizontalAlignment','center',...
        'verticalAlignment','base','FontWeight','bold','FontSize',12);
    set(gca,'Box','on','LineWidth',1.5,'XTick',[],'FontSize',10,'FontWeight','bold',...
        'LooseInset',[0 0 0 0])
    legend(group_names)
end
